...
**不动**:
  ...
  - users 主表   (display_name 不变,这样 user_id → 人名 映射保留)
...
from server.storage.models import CartItem, ChatHistory, Order  # noqa: E402
...
async def reset_demo() -> None:
    ...
    print(f"  · chat_history  删 {chat_n} 行")
    print(f"  · cart_items    删 {cart_n} 行")
    print(f"  · orders        删 {order_n} 行")
