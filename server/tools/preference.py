...
PreferenceField = Literal[
    "age", "gender", "height_cm", "weight_kg", "consumption_tier",
...
class UpdatePreferenceInput(BaseModel):
    ...
    field: PreferenceField = Field(
        description=(
            "要更新的字段名,必须在白名单内。"
            "首类字段(age/gender/height_cm/weight_kg/consumption_tier/address/recipient_name/phone)写到 profile 列;"
            "其它字段写到 preferences JSON。"
        )
    )
    value: Any = Field(
        description=(
            "字段值。类型必须匹配:age=int,height_cm/weight_kg=float,"
...
class UpdatePreferenceTool(Tool):
    ...
    description: ClassVar[str] = (
        "把用户的稳定事实(肤质 / 体型 / 偏好 / 收货信息等)写进 user_profile。"
        "只在用户明确陈述事实时调,**不要**为模糊语句 / 本次约束 / 闲聊调。"
