...
# 个性化重排(§4.3 profile resort 层):确定性 fit 加分,只在**已过阈值的合格集**内排序、
# 绝不淘汰(浏览型 query 的安全垫)。relevance(LLM)仍是主维度,fit 只做同档微调。
PROFILE_FIT_SIGNAL_WEIGHT: float = _env_float("PROFILE_FIT_SIGNAL_WEIGHT", 0.05)  # 每个命中信号 ±此值
PROFILE_FIT_BOOST_CAP: float = _env_float("PROFILE_FIT_BOOST_CAP", 0.1)  # fit 加分总封顶(±)
...
__all__ = [
    ...
    "PROFILE_FIT_SIGNAL_WEIGHT", "PROFILE_FIT_BOOST_CAP",
    # Indexing
